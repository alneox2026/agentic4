...
# Import resources only when a Terraform state was genuinely lost. This helper
# is intentionally opt-in because importing another stack's resources creates
# a destructive rename plan in the current stack.
...
GATEWAY_SERVICE_NAME="${GATEWAY_SERVICE_NAME:-${MIDDLEWARE_STACK_NAME}-gateway}"
WORKER_SERVICE_NAME="${WORKER_SERVICE_NAME:-${MIDDLEWARE_STACK_NAME}-persistence-worker}"
BILLING_API_SERVICE_NAME="${BILLING_API_SERVICE_NAME:-${MIDDLEWARE_STACK_NAME}-billing-api}"
GATEWAY_SERVICE_ACCOUNT_NAME="${GATEWAY_SERVICE_ACCOUNT_NAME:-${MIDDLEWARE_STACK_NAME}-gateway-sa}"
WORKER_SERVICE_ACCOUNT_NAME="${WORKER_SERVICE_ACCOUNT_NAME:-${MIDDLEWARE_STACK_NAME}-worker-sa}"
BILLING_API_SERVICE_ACCOUNT_NAME="${BILLING_API_SERVICE_ACCOUNT_NAME:-${MIDDLEWARE_STACK_NAME}-billing-api-sa}"
EVENTARC_SERVICE_ACCOUNT_NAME="${EVENTARC_SERVICE_ACCOUNT_NAME:-${MIDDLEWARE_STACK_NAME}-eventarc-sa}"
BILLING_RECONCILER_SERVICE_ACCOUNT_NAME="${BILLING_RECONCILER_SERVICE_ACCOUNT_NAME:-${MIDDLEWARE_STACK_NAME}-reconciler-sa}"
PUBSUB_TOPIC_NAME="${PUBSUB_TOPIC_NAME:-${MIDDLEWARE_STACK_NAME}-turn-events}"
